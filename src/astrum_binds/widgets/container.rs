...
pub fn make_container_widget(
    data: mlua::Table
) -> Container<AstrumMessages, cosmic::Theme> {
    ...
    if let Ok(width) = data.get::<_, mlua::String>("center_x") {
        container_widget = container_widget.center_x(match width.to_str().unwrap() {
            "fill" => Length::Fill,
            _ => Length::Shrink, // since shrink is default
        });

    } else if let Ok(width) = data.get::<_, mlua::Table>("center_x") {
        // covers FillPortion(i32) and Fixed(u32)
        let width_type: mlua::String = width.get(1).unwrap();
        let width_contents: mlua::Number = width.get(2).unwrap();

        container_widget = container_widget.center_x(match width_type.to_str().unwrap() {
            "fill_portion" => Length::FillPortion(width_contents as u16),
            "fixed" => Length::Fixed(width_contents as f32),
            _ => Length::Shrink
        });
    ...
    if let Ok(height) = data.get::<_, mlua::String>("center_y") {
        container_widget = container_widget.center_y(match height.to_str().unwrap() {
            "fill" => Length::Fill,
            _ => Length::Shrink, // since shrink is default
        });

    } else if let Ok(height) = data.get::<_, mlua::Table>("center_y") {
        // covers FillPortion(i32) and Fixed(u32)
        let height_type: mlua::String = height.get(1).unwrap();
        let height_contents: mlua::Number = height.get(2).unwrap();

        container_widget = container_widget.center_y(match height_type.to_str().unwrap() {
            "fill_portion" => Length::FillPortion(height_contents as u16),
            "fixed" => Length::Fixed(height_contents as f32),
            _ => Length::Shrink
        });
    ...
    if let Ok(horizontal_alignment) = data.get::<_, mlua::String>("align_x") {
    ...
    if let Ok(vertical_alignment) = data.get::<_, mlua::String>("align_y") {
