...
pub fn make_column_widget(
    data: mlua::Table
) -> Column<AstrumMessages>{
    ...
    if let Ok(alignment) = data.get::<_, mlua::String>("align_x") {
        column_widget = column_widget.align_x(match alignment.to_str().unwrap() {
    ...
    if let Ok(width) = data.get::<_, mlua::Number>("max_width") {
